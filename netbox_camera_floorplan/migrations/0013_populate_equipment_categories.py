...
def populate_categories_and_map(apps, schema_editor):
    ...
    for slug, name, is_camera, is_hub, slot_label_format in CATEGORY_DEFS:
        # Correlate primarily by NAME, not slug. EquipmentCategory.name
        # is also unique, and a category manually created during Phase 1
        # UI testing (e.g. typing "Access Control" into the Add form)
        # gets its slug auto-generated by NetBox's own slugify — which
        # may not land on exactly "access_control" (e.g. "access-control"
        # with a hyphen instead). Looking up by slug alone would then
        # miss that row entirely and attempt to create a duplicate,
        # which fails on the name uniqueness constraint instead (this
        # exact failure mode was hit against real data — name lookup
        # first avoids it regardless of whatever slug NetBox generated).
        category = EquipmentCategory.objects.filter(name=name).first()
        if category is None:
            category = EquipmentCategory.objects.filter(slug=slug).first()
        if category is None:
            ...
            if changed:
                category.save()
        # Keyed by OUR canonical slug (not category.slug, which may
        # differ from what we expect) so the CameraType mapping loop
        # below still works regardless of the actual stored slug.
